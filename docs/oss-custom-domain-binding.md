...
| `www.serverlessinsight.com` | `...ap-southeast-1.thepacificxxs.com` | 子域名同样使用阿里云 CNAME 域名 |

> 💡 **注意**：所有域名类型（根域名和子域名）现在统一使用阿里云推荐的 CNAME 域名（`taihangcda.cn` 系列），通过 `GetBucketInfo` API 自动获取正确的外网 Endpoint 并转换。
